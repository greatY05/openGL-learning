...
glm::vec3 lightPos(1.2f, 0.3f, 2.0f); // light source location
...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glm::vec3 ambientColor = ambientColor * glm::vec3(0.2f);
		...
		lightCubeShader.use();
		lightCubeShader.setVec3("color", diffuseColor + glm::vec3(0.5f));
		lightingShader.use();
